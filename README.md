# Projeto_Final_Generation

# 🌾 Inteligência Agroambiental e Créditos de Carbono
...
│
│
├── dados/                   # Camadas de Armazenamento de Dados 
│   ├── 00_raw/              # Dados brutos imutáveis (NASA, MapBiomas, SEEG, Kaggle)
    ├── 01_bronze/           # Extração das coleções utilizadas no projeto e padronização para .csv
...
Este projeto adotou uma abordagem de desenvolvimento assistido por Inteligência Artificial. Foram utilizadas ferramentas de LLM como **Google Gemini** e **Claude Opus** (via interface integrada de desenvolvimento/Antigravity) operando como co-pilotos consultivos ao longo de todo o ciclo do projeto.
...
1. **Diagnóstico Estrutural de Código:** * Análise automatizada de scripts legados para identificação de gargalos de I/O e bugs latentes.
   * Fornecimento de recomendações arquiteturais para a estruturação das pastas do repositório (`/notebooks`, `/src`, `/data`) e orquestração das fases da Arquitetura Medalhão.
